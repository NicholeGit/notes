...
// 通过多个node进行一系列的解析，得到最终结果。形成一个解析链
type ProgramNode struct {
	commandListNode node
}

func (self *ProgramNode) Parse(context *context) {
	self.commandListNode = &commandListNode{start: "program", end: "end"}
...
func (self *commandListNode) Parse(context *context) {
	...
	for {
		context.skipToken()
		if context.currentToken == self.end {
			break
		} else if context.currentToken == self.start {
			isStart = true
			continue
		} else if isStart {
